...
import {
  MapPin, Lock, CheckCircle, BookOpen, Music,
  Home, Utensils, Shirt, Loader2,
  ...
  return (
    <div className="relative min-h-screen space-y-8 pb-20 px-4 md:px-8">
      ...
      <div className="relative grid grid-cols-1 gap-8">
        ...
        <div className="relative w-full h-150 bg-[#fdfaf3] rounded-[4rem] border-4 border-cream-dark shadow-2xl overflow-hidden group">
            ...
            >
              ...
              <div className="md:col-span-5 space-y-8 relative z-10">
                <div className="flex items-center gap-6">
                  <div className={`p-6 rounded-4xl shadow-2xl ${selectedProv.userProgress?.isUnlocked ? 'bg-primary text-white' : 'bg-gray-200 text-gray-400'}`}>
                ...
                {selectedProv.userProgress?.isUnlocked ? (
                  <button
                    ...
                    className="w-full py-6 bg-primary text-white font-black text-sm uppercase tracking-[0.4em] rounded-4xl hover:bg-text transition-all shadow-2xl shadow-primary/30"
                  >
                    Mulai Quest Budaya
                  </button>
                ) : (
                  <div className="py-6 bg-gray-100 rounded-4xl text-center border-2 border-dashed border-gray-300 font-black text-gray-400 uppercase tracking-widest text-xs">
      ...
      {/* GRID 38 WILAYAH (DAFTAR KARTU PASPOR) */}
      <div className="pt-20">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6">
          {provinces.map((prov: any) => {
            ...
            return (
              <motion.button
                key={prov.id}
                onClick={() => isUnlocked && handleSelectProvince(prov)}
                disabled={!isUnlocked}
                whileHover={isUnlocked ? { y: -5, scale: 1.02 } : {}}
                whileTap={isUnlocked ? { scale: 0.95 } : {}}
                className={`relative flex flex-col items-center justify-center p-5 rounded-2xl border-2 transition-all duration-300 text-center h-36 ${
                  isUnlocked 
                    ? 'bg-white border-[#D4AF37] shadow-lg shadow-yellow-500/10 cursor-pointer group' 
                    : 'bg-cream border-cream-dark opacity-70 cursor-not-allowed grayscale-50'
                }`}
              >
                {/* Ikon Gembok / Pin */}
                <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-3 transition-colors ${
                  isUnlocked 
                    ? 'bg-yellow-50 text-[#D4AF37] group-hover:bg-[#D4AF37] group-hover:text-white' 
                ...
                {/* Nama Provinsi */}
                <h3 className={`font-bold text-sm leading-tight ${isUnlocked ? 'text-[#0a0a0a]' : 'text-gray-500'}`}>
                  {prov.name}
                </h3>

                {/* Badge Status Kecil di pojok atas */}
                <div className={`absolute top-0 right-0 translate-x-2 -translate-y-2 px-2 py-0.5 rounded-md text-[10px] font-black uppercase tracking-wider border-2 ${
                  isUnlocked 
...
function CultureCard({ icon: Icon, label, value }: { icon: any, label: string, value: string }) {
  ...
  return (
    <div className="bg-white/80 backdrop-blur p-6 rounded-4xl border-2 border-cream-dark shadow-sm hover:shadow-md transition-shadow">
